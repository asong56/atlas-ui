...
(function (global, factory) {
  ...
  const findFirstImage = () => {
    const img = document.querySelector('body img[src]');
  ...
  const setJsonLd = (data) => {
    let el = document.querySelector(`script[type="application/ld+json"][${STROMA_ATTR}]`);
    if (!el) {
  ...
  const cleanup = () => {
    document.querySelectorAll(`[${STROMA_ATTR}]`).forEach(el => el.remove());
  };

  // ─── String helpers (SSR / renderToString) ─────────────────────────────────

  const strMeta = (attrName, attrValue, content) => {
    if (!content) return '';
    return `<meta ${attrName}="${escapeHtml(attrValue)}" content="${escapeHtml(content)}" ${STROMA_ATTR}>\n`;
  };

  const strLink = (rel, href) => {
    if (!href) return '';
    return `<link rel="${escapeHtml(rel)}" href="${escapeHtml(href)}" ${STROMA_ATTR}>\n`;
  };
  ...
  const strJsonLd = (data) =>
    `<script type="application/ld+json" ${STROMA_ATTR}>\n${JSON.stringify(data, null, 2)}\n</script>\n`;

  // ─── JSON-LD Schema Builders ───────────────────────────────────────────────

  const buildWebPageSchema = (cfg) => {
    const s = {
  ...
  const buildBreadcrumbSchema = (items) => ({
    ...
    'itemListElement': items.map((item, i) => ({
      '@type': 'ListItem', 'position': i + 1, 'name': item.name, 'item': toAbsoluteUrl(item.url),
    })),
  });

  const pickSchema = (cfg) => {
    switch (cfg.schema) {
  ...
  const resolveConfig = (input) => {
    ...
    const maxT = cfg.maxTitleLength       || DEFAULTS.maxTitleLength;
    const maxD = cfg.maxDescriptionLength || DEFAULTS.maxDescriptionLength;
  ...
  const applyTags = (cfg) => {
    for (const tag of describeTags(cfg)) {
      switch (tag.fn) {
  ...
  const renderTags = (cfg) => describeTags(cfg).map((tag) => {
    switch (tag.fn) {
